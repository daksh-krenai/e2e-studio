...
function runClaudeAgent(prompt, cwd, runId, emit) {
  return new Promise((resolve, reject) => {
    ...
    let cmd = 'claude'
    let args = [
      '-p', 'Execute the E2E test plan provided via standard input.', 
      '--allowedTools', 'Bash,Read,Write',
      '--dangerously-skip-permissions'
    ]
    ...
    if (process.platform === 'win32') {
      ...
      try {
        ...
        emit(`⚠️ [System] Claude not found in PATH. Falling back to npx executor...`)
        cmd = 'npx.cmd'
        args = ['-y', '@anthropic-ai/claude-code', '-p', 'Execute the E2E test plan provided via standard input.', '--allowedTools', 'Bash,Read,Write', '--dangerously-skip-permissions']
      }
    }
    ...
    // Stream the guardrailed prompt directly into Claude's standard input
    proc.stdin.write(guardrailedPrompt)
    proc.stdin.end()
      ...
      if (isError) {
        ...
        lines.forEach(line => {
          ...
          if (cleanLine.toLowerCase().includes('running command') || cleanLine.toLowerCase().includes('tool use')) {
            ...
            if (bashCommandCount > 12) {
              proc.kill('SIGKILL');
              return reject(new Error(`🛑 Loop Protection Triggered: Agent exceeded 12 bash commands. Terminated to save costs.`));
            }
          }
        })
      }
